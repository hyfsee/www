...
include('../config.php');
 			 ...
 			 $sql="insert into story(Id,title,auther,picture,type,date,product,content,pageviews,collectionname,collection,ranking) values (null,'$title','$auther','$picture','$type','$date','$product','$content','$pageviews','$collectionname','$collection','$ranking')";//向数据库插入表单传来的值的sql
 			 $reslut=mysqli_query($conn,$sql);//执行sql
		     if($reslut){
				echo '\n'+'发布成功,<a href="../uploadfile/index.html">再写一篇</a>';
			 }else{
